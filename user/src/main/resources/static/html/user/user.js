...
function doSearch() {
    ...
    var ajaxParm={"name_LIKE":name,"realurl":"user-service/sys/selectSysUser"};
    if(user.appid!=2){
        //非用户中心管理员，查询条件新增appid
        ajaxParm=$.extend(ajaxParm,{"appid":user.appid});
    }
        ...
        form.on("submit(addSubmitFilter)",function(data){
            ...
            if (pwd1==pwd2 && pwd1!=null){
                ...
                baseTools.xhrAjax({
                    url: "../../doService",
                    contentType: "application/json",
                    params:  JSON.stringify({"account":data.field.account,"appid":appid,"realurl":"user-service/sys/selectSysUser"}),
                    callback: [function (jsonObj, xhrArgs) {
                                    ...
                                    params:  JSON.stringify(
                                        {
                                            ...
                                            "realurl":"user-service/sys/insertSysUser"
                                        }),
                                    callback: [function (jsonObj, xhrArgs) {
                                        switch (parseInt(jsonObj.code)) {
                                            case 0://成功
