...
var baseTools = (function () {
    ...
    return {
    	...
    	gotoLogin: function (dateStr) {
    		...
    		top.location.href="/user/html/login/login.html?appid="+appid;
        },
    	//解决IE8获取时间失败
        ...
        //id,pid结构转tree结构
        getTreeDataByIdPid:function(jsonData){
            var treeData=[];
            for(var i in jsonData){
                if(jsonData[i].pid==-1){
                    var tempObject={};
                    tempObject.title=jsonData[i].name;
                    tempObject.id=jsonData[i].id;
                    tempObject.url=jsonData[i].url;
                    tempObject.children=baseTools.getChildren(tempObject.id,jsonData);
        ...
        //递归获取孩子
        getChildren:function(id,jsonData){
            var tempArray=[];
            for(var i in jsonData){
                if(jsonData[i].pid==id){
                    ...
                    tempChild.url=jsonData[i].url;
                    if(baseTools.selectChildren(jsonData[i].id)){   //若存在子节点，继续递归；否则为叶节点，停止递归
                        tempChild.children=baseTools.getChildren(jsonData[i].id,jsonData);
                    }
        ...
        // 是否存在子节点
        selectChildren:function(id,jsonData){
            for(var i in jsonData){
                if(jsonData[i].pid==id){
                    return true;
                }
      ...
      ///fwzs
           var loading = '/user/images/load/loading.gif';
           ...
           // var urlParam = this.preparePostData(xhrArgs.params);
            ...
            $.ajax({
                ...
                complete: function (XMLHttpRequest, textStatus) {
                	...
                	$.cookie("x-csrf-token", val, {path: "/user/"});
